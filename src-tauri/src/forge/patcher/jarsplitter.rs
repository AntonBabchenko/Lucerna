...
pub async fn run(args: Vec<String>, _ctx: &ProcessorContext) -> Result<()> {
    let input = read_flag(&args, "--input")
        .ok_or_else(|| patcher_fail("jarsplitter", &"missing --input"))?;
    let slim =
        read_flag(&args, "--slim").ok_or_else(|| patcher_fail("jarsplitter", &"missing --slim"))?;
    let extra = read_flag(&args, "--extra")
        .ok_or_else(|| patcher_fail("jarsplitter", &"missing --extra"))?;
    let srg_path =
        read_flag(&args, "--srg").ok_or_else(|| patcher_fail("jarsplitter", &"missing --srg"))?;

    let srg_text = tokio::fs::read_to_string(&srg_path)
        .await
        .map_err(|e| patcher_fail("jarsplitter", &format!("read {srg_path}: {e}")))?;
    let known = parse_class_set(&srg_text);

    let bytes = tokio::fs::read(&input)
        .await
        .map_err(|e| patcher_fail("jarsplitter", &format!("read {input}: {e}")))?;
    let mut archive = zip::ZipArchive::new(std::io::Cursor::new(bytes))
        .map_err(|e| patcher_fail("jarsplitter", &format!("zip open {input}: {e}")))?;
    ...
    let slim_file = std::fs::File::create(&slim)
        .map_err(|e| patcher_fail("jarsplitter", &format!("create {slim}: {e}")))?;
    let extra_file = std::fs::File::create(&extra)
        .map_err(|e| patcher_fail("jarsplitter", &format!("create {extra}: {e}")))?;
    ...
    for i in 0..archive.len() {
        let mut entry = archive
            .by_index(i)
            .map_err(|e| patcher_fail("jarsplitter", &format!("zip index {i}: {e}")))?;
        if entry.is_dir() {
            continue;
        }
        ...
        entry
            .read_to_end(&mut buf)
            .map_err(|e| patcher_fail("jarsplitter", &format!("read entry {name}: {e}")))?;

        let goes_to_slim = name.ends_with(".class") && {
            let class_fqn = name.strip_suffix(".class").unwrap_or(&name);
            known.contains(class_fqn)
        };
        let target = if goes_to_slim {
            &mut slim_zip
        } else {
            &mut extra_zip
        };
        target
            .start_file::<_, ()>(name.clone(), opts)
            .map_err(|e| patcher_fail("jarsplitter", &format!("zip start {name}: {e}")))?;
        target
            .write_all(&buf)
            .map_err(|e| patcher_fail("jarsplitter", &format!("zip write {name}: {e}")))?;
    }
    slim_zip
        .finish()
        .map_err(|e| patcher_fail("jarsplitter", &format!("slim finish: {e}")))?;
    extra_zip
        .finish()
        .map_err(|e| patcher_fail("jarsplitter", &format!("extra finish: {e}")))?;
    Ok(())
}
...
mod tests {
    ...
    async fn jarsplitter_routes_classes_correctly() {
        ...
        let args = vec![
            ...
            "--slim".into(),
            slim_path.display().to_string(),
            "--extra".into(),
            extra_path.display().to_string(),
            "--srg".into(),
            srg_path.display().to_string(),
        ];
