...
async fn full_chain_smoke_via_test_seam() {
    ...
    let (xbl_tok, _) = lucerna_lib::accounts::microsoft::__test_xbl_authenticate("ms-tok")
        .await
        .unwrap();
    let xsts = lucerna_lib::accounts::microsoft::__test_xsts_authorize(&xbl_tok)
        .await
        .unwrap();
    let login = lucerna_lib::accounts::microsoft::__test_login_with_xbox(&xsts.0, &xsts.1)
        .await
        .unwrap();
    let profile = lucerna_lib::accounts::microsoft::__test_fetch_profile(&login.0)
        .await
        .unwrap();
...
fn account_file_v1_migration_happy_path() {
    use lucerna_lib::accounts::store::{read_account_file, AccountKind};
...
async fn full_chain_403_invalid_app_registration_maps_to_pending_approval() {
    ...
    let ms_token = lucerna_lib::accounts::microsoft::oauth::exchange_code_for_token(
        "test-code",
        "test-verifier",
    ...
    let (xbl_token, _uhs) =
        lucerna_lib::accounts::microsoft::xbox::xbl_authenticate(&ms_token.access_token)
            .await
            .expect("xbl");

    let xsts = lucerna_lib::accounts::microsoft::xbox::xsts_authorize(&xbl_token)
        .await
        .expect("xsts");
