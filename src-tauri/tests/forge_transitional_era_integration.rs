...
fn load_or_skip() -> Option<(Vec<u8>, serde_json::Value)> {
    let bytes = match std::fs::read(FIXTURE) {
        Ok(b) => b,
        Err(_) => { eprintln!("SKIP: 1.16.5 fixture absent"); return None; }
    };
    let cursor = std::io::Cursor::new(bytes.clone());
    let mut archive = zip::ZipArchive::new(cursor).expect("zip");
    let mut entry = archive.by_name("install_profile.json").expect("install_profile");
    use std::io::Read;
...
fn install_profile_has_4_processors() {
    ...
    assert_eq!(parsed.processors.len(), 4, "expected 4 processors per audit");
}
...
fn every_processor_coord_recognised() {
    ...
    for p in &parsed.processors {
        ...
        assert!(recognised.iter().any(|(rg, ra)| *rg == g && *ra == a),
            "unrecognised processor coord: {} — extend run_processor", p.jar);
    }
}
...
async fn substitute_args_reserved_placeholders() {
    ...
    let raw = vec!["{ROOT}".into(), "{INSTALLER}".into(), "{SIDE}".into(), "{MINECRAFT_JAR}".into()];
    let out = substitute_args(&raw, &data, "client", &libs, b"", "/inst.jar", "/cache", "/mc.jar").await.unwrap();
    assert_eq!(out, vec!["/cache".to_string(), "/inst.jar".into(), "client".into(), "/mc.jar".into()]);
}

#[tokio::test]
async fn substitute_args_quoted_hex_literal() {
    let mut data = HashMap::new();
    data.insert("HASH".into(), DataEntry {
        client: "'deadbeef'".into(),
        server: "'cafebabe'".into(),
    });
    let libs = PathBuf::from("/x/libraries");
    let out = substitute_args(&vec!["{HASH}".into()], &data, "client", &libs, b"", "/i", "/c", "/m").await.unwrap();
