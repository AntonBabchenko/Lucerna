...
<div class="rounded-xl border border-border-subtle bg-surface p-3.5 flex flex-col gap-2">
  <h3 class="text-[10px] uppercase tracking-wider text-muted">
    {$t('servers.overview.connectionCard')}
  </h3>
    ...
    {:else}
      <div class="flex flex-col gap-3">
        ...
        {#if firewall === 'needs_rule'}
          <div
            class="rounded-xl border border-warning-text bg-warning-bg p-3 flex items-start gap-2"
          >
        ...
        <!-- Over the internet (#6): public IP + port-forward guidance, plus the
             online-mode explainer — behind a disclosure; port-forwarding is the
             advanced path and would otherwise dominate the card. -->
        <details class="mt-1">
          <summary
            class="inline-flex items-center cursor-pointer text-sm text-secondary select-none"
          >
            <span class="disclosure-caret mr-1"><Icon name="caret" size={14} /></span>
            {$t('servers.connect.internetTitle')}
          </summary>
          <div class="mt-2 flex flex-col gap-3">
            <div data-testid="server-connect-internet">
              ...
              {:else}
                ...
                <p class="text-muted text-xs">
                  {$t('servers.connect.portForwardGeneric', { port })}
                </p>
              {/if}
            </div>
