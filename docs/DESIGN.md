...
| `--danger` | `220 38 38` (red-600) | `239 68 68` | Stop / Delete / error red: `.btn-danger`, `.btn-icon-danger` hover. Stays the FILL token: the split below is text-utilities-only, so retargeting it would repaint every danger button, border and background rather than fix a foreground. |
| `--danger-text` | `220 38 38` (red-600) | `248 113 113` (red-400) | Danger **text** (`text-danger`). Split from `--danger` because the dark fill tier is only ~4.0:1 on `--bg-surface`; the solid `.btn-danger` fill keeps `--danger`. First fill/text split in the palette — the amber family instead reuses one token (`--warning-text`) for both. **Contrast is scoped to `--bg-surface`** (~4.8:1 light, ~5.5:1 dark). On a `bg-danger-bg` box it is ~4.4:1 light and ~3.6:1 dark, so the soft-error-box recipe in §10 still misses AA — closing that needs a lighter foreground (red-300 is ~5.3:1 on red-900) or a darker box, and neither is done yet. |
...
- **Rule for new dialogs:** default the title to `text-base font-semibold`; reserve `text-lg` for full-screen-replacing flows (the import/export wizards). `DialogTitle.svelte` (§8) encodes this — it defaults to `text-base` and takes `size="lg"` only for those flows. The `text-lg` confirm dialogs above predate the rule and pass `size="lg"` explicitly — don't copy them as precedent.
...
- **`HelpPopover` (`(?)`) is for conceptual help next to a header — never to name an action.** It has two modes, differing only in length and width: a **one-sentence helper** at the default 260px (the Sidebar's "Instance" header, the modpacks modal title), and a **concept explainer** at 320px — several paragraphs answering "what IS this thing", used where a feature has a name a user may not know (data packs, mod translations). Both take one `paragraphs` array, one `<p>` per entry; the concept mode is always reached through `ConceptHelp` (§8), never by hand. A hover tooltip names a control tersely; a HelpPopover explains a control; a `LogHintCard` explains a recognized log line — a rich hover card that persists while hovered (its text can be selected and copied), deliberately not `role="tooltip"` so the app's single tooltip bubble stays a singleton. Keyboard path: the gutter badge is focusable (`:focus-visible`-gated) and Escape closes the card without closing the hosting modal.
...
- **`SegmentedControl`** — choose-one-of-N with all options visible. `variant="boxed"` is a bordered pill group whose segments *swap* the variant class conditionally (active = `.btn-primary`, inactive = `.btn-ghost`) — never stack two `btn-*` purpose classes on one element: at equal specificity the later `app.css` rule wins the cascade and kills the active fill. Used by the grid/list `LayoutToggle`; `variant="inline"` is bare clickable text numbers, used by `PageSizePicker`. Activation follows focus (options act immediately). Use over `Select` when the option set is short and worth showing. `ModeSwitcher` (`src/lib/layout/ModeSwitcher.svelte`) is the top-level Client/Servers mode switch — a dedicated segmented control whose servers segment carries live status content (`NavStatusIcon` + badges), deliberately not hideable and not built on `SegmentedControl`.
...
- **Never disable the focused control inside a `Modal`.** `trapFocus` skips natively-disabled buttons and its Tab handler lives on the panel, so a focused button that turns `disabled` drops focus to `<body>` and Tab starts walking the page behind the dialog. Either keep the control focusable (`aria-disabled` + a click guard) or move focus to the panel first — `BusyButton` disables natively while busy, so a handler that is about to make its own button busy parks focus on the dialog body before it does. The same applies to a focused control that is **removed** (an `{#if}` that turns false): after the update, if focus has left the dialog, put it back on the panel. `DataLocationProgressDialog` is the reference: its Cancel leaves with the phase, and it also moves focus to its primary action when the dialog changes from *running* to its final state.
- **App-level blocking dialogs.** A dialog that must survive a page reload or a tab switch (the data-folder move: its final state is the only way to restart) is not component state: a store holds the view, and a self-gating host mounted last in `+page.svelte` renders it (`DataMoveHost`, after the `WhatsNewModal` precedent).
- **`bare` variant:** full-bleed dialogs (the screenshot lightbox) pass `bare` — the panel fills the viewport without surface chrome and the scrim darkens to `bg-black/60`, while the Escape stack, focus trap, and role/aria wiring stay. The panel covers the backdrop, so a bare dialog provides its own click-to-close surface inside.

**Confirm-dialog convention.** `ConfirmDialog.svelte` is the shared confirm primitive — a thin presentational wrapper over `Modal` (the mutation stays with the caller via `onConfirm`). It renders a `<DialogTitle>` (§3), either `bodyText` (a `text-sm text-secondary` string or array of stacked paragraphs) or a `body` snippet for richer copy, and a right-aligned footer: a `.btn-secondary.btn-sm` Cancel plus a `variant`-driven confirm (`danger` → `.btn-danger`, `primary` → `.btn-primary`) at `.btn-sm`, whose label *is* the verb for destructive actions. It wires the body's id through Modal's `aria-describedby`, so every confirm announces its body copy, not just its title. Setting `busy` locks out backdrop/Escape (`closeOnBackdrop`/`closeOnEscape`) and disables the confirm while the mutation runs. New confirm dialogs wrap `ConfirmDialog` rather than re-deriving this shape; `RepairConfirmCard` is the one deliberate exception (an inline card, not a `Modal`). Whole-instance deletion (`ManageInstancesModal`) keeps a stronger type-to-confirm gate inline, given its larger blast radius.

**Fixed-popover family.** `OverflowMenu`, `ContextMenu`, `HelpPopover`, `Select`, `LogHintCard`, and the tooltip layer all use `position: fixed` to escape host `overflow` boxes, measure from the trigger on open, clamp into the viewport (8px margin), and close on captured `scroll` + `resize`. `Select`, `Menu`, and `HelpPopover` share that close-on-scroll/resize wiring via `attachPopoverDismiss` (`popover-dismiss.ts`), whose `ignoreScrollWithin` keeps an internally-scrolling list from dismissing itself; their surface z-index is one token, `var(--z-popover)` (also consumed by `McVersionCombobox`).

- **Menus** (`OverflowMenu` left-click ⋯, `ContextMenu` right-click) render through one shared `Menu.svelte` — the `role="menu"` / `role="menuitem"` surface, arrow-key roving `activeIndex`, Escape close, and scroll/resize dismiss. Both wrappers own only their trigger, positioning, and focus-return; they share the item model `ContextMenuItem` (`label`, `icon?`, `danger?`, `disabled?`, `disabledReason?`, `separatorBefore?`, `onSelect`), which lives in `ui/menu-item.ts` so plain modules can build menus. A disabled item that needs explaining sets `disabledReason`: `Menu` renders it as a second line under the label, not as a tooltip — arrow keys skip disabled items, so a hover-only reason never reaches a keyboard user. Escape is stopped at the menu, so a `Modal` hosting one stays open. Conventions for a per-row menu: a right-click does not move the selection (the menu acts on its own row); an item that mirrors a control reuses that control's label key; the `aria-label` names the row ("Actions for {name}"); the destructive item comes last, behind a separator. A double-click on a row, where offered, runs the row's default action — the first item of its menu — and only when that action is cheap and reversible (the Manage list: make the profile active); the menu item stays as the keyboard path.
...
- **`HelpPopover`** is a click-toggle `(?)` helper with persistent text — distinct from the hover tooltip. It takes a single required `paragraphs: readonly string[]` (one `<p>` per entry), which is what keeps its two modes (§5: one sentence at 260px, a concept panel at 320px) one component rather than two: an empty popover and a "both props set" ambiguity are both unrepresentable. Escape closes it and returns focus to the trigger; click-outside, scroll and resize also dismiss it.
- **`ConceptHelp`** (`src/lib/onboarding/`) is the onboarding binding over `HelpPopover`, and how every explanation-level-aware `onboarding.*` explainer is built. A caller names an i18n namespace and lists its paragraph leaves — `<ConceptHelp namespace="onboarding.datapackConcept" paragraphKeys={['p1', 'p2', 'p3', 'p4']} />` — and the wrapper resolves the explanation-level (Basic / Advanced) swap, the trigger/close strings, and the 320px width. Both arguments are checked against the generated `TranslationKey` union, so a mistyped namespace or a missing paragraph is a compile error rather than a raw key on screen. The named per-concept wrappers (`DatapackConceptHelp`, `L10nConceptHelp`, `InstanceConceptTooltip` — a popover despite the name) are one such tag each, and exist only so a concept used on several surfaces cannot drift between them. `ModpacksModal` is the one surface still calling `HelpPopover` directly — a single header sentence, no multi-paragraph copy to keep in step. The layering is deliberate: `src/lib/ui/` stays free of any i18n or onboarding dependency.
...
- **`CountPill`** is the rounded-full **numeric** badge — the sidebar's modpack-update count and the running-instance / running-server counts in the app chrome. It carries only a size (`sm` = 15px for the compact ModeSwitcher rail, `md` = 18px for an inline label row), because a count is always the `--success` tone; a pill carrying a **word** is a status and belongs in `StatusBadge` (that is why Manage's "Active" marker is a badge, not a pill). Like `ToggleChip` it also exports `countPillClass(size)`, so the two running-count popovers — whose pill *is* their `<button>` trigger, and so cannot nest the `<span>` the component renders — get the identical look without losing the button semantics. Its `text-white` label on `bg-success` is inherited verbatim from the three hand-rolled copies it replaced and does not clear AA (see Known gaps); the extraction deliberately changed no pixels.
...
**Read-only cards can be click targets.** A card whose body is a set of values (Overview's Configuration / Mods / Integrity) may route the user to where those values are edited. Because a `<button>` cannot nest another `<button>`, "the whole card is clickable" is expressed as adjacent full-width `.card-zone` buttons that carry the card's padding themselves — the card container drops its own `p-*` and the zones tile its interior. `.card-zone` supplies the hover fill and an *inset* focus ring (the card clips overflow for its rounded corners). Real actions — Optimise, Export — stay `.btn-*` buttons in their own row outside the tiled surface; only navigation becomes a zone. When a zone leads into a modal aimed at one control, the modal wraps that control in `use:fieldFlash` (`src/lib/ui/field-flash.ts`), which scrolls it into view and paints `.field-flash` for 2s. `fieldFlash` moves keyboard focus only when told to: never onto a range slider, where a stray arrow key would silently rewrite the value. When it does, it prefers a descendant marked `data-flash-focus` (the control the user came for — a key input, not the link before it) over the first focusable, opens any closed `<details>` around it *before* measuring and scrolling, and, if that control is still `disabled` (a field waiting on a read), parks focus on the wrapper and hands it to the control the moment it enables — unless something else claimed focus meanwhile. A block taller than its scroll column aligns to its start (`block: 'start'`), so a long section shows its heading, not its middle; the wrapper carries `scroll-mt-4` so the ring stays visible. The action reports delivery (`onDelivered`, in a microtask) so the caller can consume its jump once; deactivating the action lets a running ring finish.

**A zone carries an `aria-label` only when its visible text is a poor name on its own.** `aria-label` *overrides* the accessible name, so putting one on a zone that renders live content — mod counts, an integrity status that varies across three states — silences exactly what a sighted user is reading. Bare uppercase eyebrow headers (`CONFIGURATION`, `MODS`) do need one; content-bearing zones name themselves. Where a label is kept on a value row anyway (Configuration's Minecraft / Loader / Memory, whose visible text does not convey that clicking edits it), it must *include* the value — and the label and the visible span must read the same expression, so the two cannot drift.
...
- **A gate banner with actions** — one whose condition blocks other actions and that therefore has no × (the recovery-session banner, `DataRootFallbackBanner`) — puts its actions UNDER the text, both `.btn-sm`: the way forward as solid `.btn-warning`, the secondary one (Restart) as `.btn-warning-soft` in a `BusyButton`. A failed action is shown inside the banner as a `text-danger` line WITHOUT its own `role="alert"`: the banner root already is one, and a nested live region re-announces the whole banner. This is a deliberate departure from `ServerDiagnosisBanner`, whose action-error lines DO carry a nested `role="alert"` — the older pattern, kept there for now.
- **A setting whose value could not be read shows no value.** A control is tri-state: *pending* (disabled, no value) until the read settles, *ok* with the confirmed value, *failed* (disabled, no value) with a `StatusMessage` naming the reason and a **Retry**. A failed save reverts to the value the backend last confirmed and says the change was not saved. Never a hard-coded default next to a red line — an unknown shown as a confident "on". Reference: `UpdatesPanel`'s startup-check toggle; the other Settings pages follow in the save-contract pass.
- **Every instant-apply setting is saved through `patchGeneral`** (`src/lib/settings/app-settings.svelte.ts`): one field per change, optimistic on top of the last confirmed block, rolled back only where still displayed, and the failure named next to the control (`saveFailure(field)`, in a `data-testid="save-failure-<field>"` wrapper). A consent's failed revoke says the channel is still on only when the backend *refused* (the file is unchanged); a call the transport lost says the change could not be confirmed and the block is re-read.
- Within one banner: container is soft, the primary CTA may be solid.

**Diagnosis banner anatomy.** `Banner.svelte` is the shared soft-severity banner shell: a `rounded-[var(--radius-lg)] border bg-* p-3` container laid out `flex items-start gap-2` — a leading tone-coloured `<Icon>`, a `flex-1` body, and an optional top-right dismiss ×. Severity comes from the §1 tone families (`tone` prop). `LogDiagnosisBanner` and `ServerDiagnosisBanner` render it with `tone="warning"` + `role="alert"`, a bold `text-warning-text` title, a `text-sm` body, a "what to try" line, and the action (a `BusyButton.btn-primary.btn-sm`, rendered only when the diagnosis is actionable); the caller keeps the signature-keyed dismiss/restore logic and passes `onDismiss` (Banner renders the × and calls it).
...
**API-key fields.** Both Settings key forms (CurseForge, AI translation) render their own `h3` and intro first (as every Settings section does), then `ApiKeyField` (`src/lib/settings/ApiKeyField.svelte`) in one order: status line (+ a muted detail line, + an inline action such as *Check again*) → optional `<details>` disclosure → guide (steps, a "Get a key" link) → `type="password"` field (`autocomplete="off" spellcheck="false"`; Enter saves) → `.btn-primary.btn-sm` Save / `.btn-secondary.btn-sm` Clear (+ a caption) → one `StatusMessage` result → the keyring note. Two facts, two elements: the status line says what is *stored* (the inline-status families above; `set_builtin` is quiet `text-secondary`, never success — it is not the user's key), the result says what *happened to what was just typed* (danger for a rejection, warning for "not saved", info for "saved"); a failed candidate never changes the status line. While the status loads, the status element is one `Spinner` with the label as its text — no second sr-only "Loading…". `disabled` gates the input, Save and Clear together. Guide links are `.btn-link` + trailing `externalLink`; the CurseForge console links keep `btn-tertiary font-mono` as a pinned legacy (INT-10).
...
- **The operations centre** is the bottom-strip task surface, mounted in the app shell's row-2 status slot (§14). `OperationsBar.svelte` is the single visible anchor for every task the registry tracks (game install, mod install/update, pack import/update, launcher import, clone, verify/repair, server upload, app update, data migration): it shows the earliest-started active task's `Spinner` + phase + progress bar, or a finished-tasks summary once nothing is active. `registry.svelte.ts` (`src/lib/tasks/registry.svelte.ts`) is the module-singleton store behind it — a task carries a `TaskLane`: `serial` (one at a time, FIFO-queued and reorderable while queued — verify, repair, clone, pack-import, pack-update, launcher-import), `concurrent` (runs alongside anything else — game install, mod install/update, server upload), or `modal` (owns a blocking dialog, never queued, and never cancellable **from the strip or the panel** — data-root migration, app update; the owning dialog may still offer its own cancel while that is safe, as the data-folder move does during its copy and verify phases). A task's `caps: {cancellable, reorderable}` is derived purely from its `state`/`lane`, never stored independently, so it can't drift from what it describes.
- The strip's disclosure toggle opens **`OperationsPanel.svelte`** — Running / Queued / Finished sections, each row's move-up/down and cancel controls rendered straight from `task.caps`: a running task shows neither (cancelling a running op is out of scope — no backend cancellation token exists), a queued `serial` task can be reordered, and any queued non-`modal` task can be cancelled. A row's "Details" button appears only once `task.details` is non-null — i.e. the task actually produced a file report — and opens **`TaskReportModal.svelte`**, the filterable per-file install report.
...
- **Keyboard-operable custom controls.** Any non-`<button>` element given `role="button"` + `tabindex` must wire an `Enter`/`Space` `keydown` handler so it works without a mouse — `FileDropzone` and the `WorldsTab` row are the reference implementations; no known click-only debt remains for this pattern.
...
- **Dialog title size is inconsistent.** Some dialogs use `text-base font-semibold`, others `text-lg font-semibold`. §3 sets the forward rule and `DialogTitle.svelte` now encodes it (default `text-base`, opt-in `size="lg"`); the migrated confirm dialogs use it, but the remaining inline `font-semibold` headings across the app are not yet reconciled to the component.
...
- **Banner recipes are now shared.** `Banner.svelte` unifies the soft-severity banner container (one `--radius-lg` radius, one full-opacity warning border) across `LogDiagnosisBanner` / `ServerDiagnosisBanner` / the CurseForge-key gate — replacing the drift between the diagnosis banners' `rounded-xl` full border and the gate's hand-rolled `rounded` `border-warning-text/30`. Still outside it by design: nested repair cards keep their quieter `border-warning-text/40` recipe (§10), and the top-strip `DataRootFallbackBanner` is a `border-b` bar, not a card. The `.btn-warning-soft` *button* class remains lightly used.
...
- **Duplicated primitives.** Largely resolved: `ConfirmDialog` collapses the near-identical confirm dialogs (§8); `nextRovingIndex` (`roving.ts`) shares the arrow-key math across `TabBar` / `SegmentedControl` / `ToggleChipGroup`; `attachPopoverDismiss` (`popover-dismiss.ts`) shares the fixed-popover close-on-scroll/resize across `Select` / `Menu` / `HelpPopover`; `ConceptHelp` (§8) collapses the three near-identical concept explainers, which had already drifted apart on naming, into one type-checked wrapper; and `OverflowMenu` / `ContextMenu` now render through one shared `Menu`. Remaining: `McVersionCombobox` is still `position: absolute`, so it lacks `Select`'s fixed viewport-clamp / flip-up, and it has no typeahead — though its `aria-selected` now tracks the committed value (was the keyboard-active row, a latent a11y bug `Select` doesn't have) and it has Home/End parity with `Select`.
- **Ad-hoc literals.** `font-mono text-xs`, `text-[10px]`, and `text-[0.9em]` bypass the scale tokens; uppercase label micro-caps use competing colour/tracking conventions (`text-muted` + `tracking-wide` vs `text-secondary`, and `tracking-wide` vs `tracking-wider`). Popover z-index is now unified on `var(--z-popover)` across `Select` / `Menu` / `HelpPopover` / `McVersionCombobox` (was split `z-50` / `z-40` / `z-30`). The motion (`--duration-*` / `--ease-*`) and radius (`--radius-*`) tokens exist but their inline-literal consumers still await the Banner / motion pass.
...
- **`HelpPopover` is less keyboard-accessible than `Modal`.** It has no focus trap and doesn't auto-focus its `CloseButton`, so a keyboard user must tab forward into the panel. Escape *does* close it and return focus to the trigger (this entry claimed otherwise until the `ConceptHelp` pass; the handler is in `HelpPopover.svelte`, deliberately bespoke rather than routed through `attachPopoverDismiss` because only the keyboard path restores focus).
...
- **Danger text inside a danger box still misses AA.** Splitting `--danger-text` off the fill token (§1) fixed `text-danger` on `--bg-surface`, which is where most of it renders, but the soft-error-box recipe pairs it with `bg-danger-bg` — and that combination clears neither theme. §1 records the measured ratios; fixing it means a lighter foreground or a darker box, so it is a palette decision rather than a token rename. `tests/intent/design-tokens.test.ts` deliberately asserts only the surface pairing, since a guard can only lock in a contract the palette actually meets. Two siblings in the same family miss it for the same reason and are equally unfixed: `.btn-danger`'s white label on the dark `--danger` fill is 3.76:1 (AA wants 4.5 at `text-sm`), and `.btn-icon-danger` on a hovered row is 2.76:1 against `--bg-subtle` — under even the 3:1 WCAG allows non-text content. All three want a palette decision, not a token rename. A fourth joined them with `CountPill` (§9): its `text-white` on `--bg-success` is ~3.3:1 light and ~1.9:1 dark. The pill predates the primitive — extracting it preserved the value rather than silently retuning a colour on three surfaces at once — and the same palette decision covers it.
