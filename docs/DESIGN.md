...
- **Duplicated primitives.** Roving keyboard nav is copy-pasted across `TabBar` / `SegmentedControl` / `ToggleChipGroup`; the fixed-popover close-on-scroll/resize logic is reimplemented in three overlays; `OverflowMenu` and `ContextMenu` render byte-identical menu markup. `ConfirmDialog` now collapses the near-identical confirm copies (see §8). `McVersionCombobox` still duplicates `Select`'s popover logic *without* the viewport-clamp fix and lacks typeahead — but its `aria-selected` now tracks the committed value (was the keyboard-active row, a latent a11y bug `Select` doesn't have) and it has Home/End parity with `Select`.
- **Ad-hoc literals & z-index.** `font-mono text-xs`, `text-[10px]`, and `text-[0.9em]` bypass the scale tokens; uppercase label micro-caps use competing colour/tracking conventions (`text-muted` + `tracking-wide` vs `text-secondary`, and `tracking-wide` vs `tracking-wider`). The layering / motion / radius scale tokens now exist (`--z-*`, `--duration-*`, `--ease-*`, `--radius-*` in `app.css`), but consumption is pending: popover z-index is still split (`Select` listbox `z-50`, `McVersionCombobox` `z-30`) and durations/easings are still inline literals until the popover-engine and Banner work migrate them.
