...
| `--danger` | `220 38 38` (red-600) | `239 68 68` | Stop / Delete / error red: `.btn-danger`, `.btn-icon-danger` hover. Icon foregrounds stay on this fill token rather than `--danger-text`: an icon is non-text content, which WCAG holds to 3:1, and red-500 clears that at ~4.0:1 on `--bg-surface`. The split below is text-utilities-only. |
...
- **Danger text inside a danger box still misses AA.** Splitting `--danger-text` off the fill token (§1) fixed `text-danger` on `--bg-surface`, which is where most of it renders, but the soft-error-box recipe pairs it with `bg-danger-bg` — and that combination clears neither theme. §1 records the measured ratios; fixing it means a lighter foreground or a darker box, so it is a palette decision rather than a token rename. `tests/intent/design-tokens.test.ts` deliberately asserts only the surface pairing, since a guard can lock in a contract the palette meets.
