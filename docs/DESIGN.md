...
**Fixed-popover family.** `OverflowMenu`, `ContextMenu`, `HelpPopover`, `Select`, and the tooltip layer all use `position: fixed` to escape host `overflow` boxes, measure from the trigger on open, clamp into the viewport (8px margin), and close on captured `scroll` + `resize`. That close-on-scroll/resize wiring is shared via `attachPopoverDismiss` (`popover-dismiss.ts`), whose `ignoreScrollWithin` keeps an internally-scrolling list from dismissing itself. Their surface z-index is one token, `var(--z-popover)`.
