...
1. **Reproducible builds are a goal, not a release blocker.** The target: any third party can rebuild the binary from a tagged commit and get the same SHA256. Tauri has known sources of nondeterminism (embedded assets, bundle timestamps); these are documented as they are discovered and the gaps are closed over time. Until full reproducibility is achieved, the pipeline still ships — the gap is honest, not hidden. Three build inputs identify the build and are baked into it: `LUCERNA_BUILD_TAG`, `LUCERNA_BUILD_COMMIT` and `LUCERNA_BUILD_REPO` — the tag, commit and repository the release workflow ran for, shown in Settings → About. A rebuild must set the same three values to get the same binary.
...
6. **Content Security Policy applies only to production builds — and `img-src` is deliberately broad.** The CSP declared in `src-tauri/tauri.conf.json` (`default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; …`) is enforced on the `asset://` custom protocol used by `tauri build`. **It is NOT injected into `pnpm tauri dev`**, which loads `http://localhost:1420` via Vite without the CSP header, so dev mode exercises no CSP at all. Know what the production CSP does and does not restrict. `connect-src` stays `'self' ipc: http://ipc.localhost`, so a programmatic request from webview code to a new host fails in production even where dev mode allowed it — smoke-test any such resource against a `tauri build` artifact before submitting. `img-src https:`, by contrast, admits an `<img>` from **any** https host, so a new image CDN never trips a CSP violation in either mode — that is not an oversight but a sanctioned outbound channel with its own bounds, documented in `docs/PRINCIPLES.md` Part A, commitment 6.
...
4. **Keyring backends.** `keyring` is built with `windows-native` on Windows, `apple-native` on macOS and `sync-secret-service` + `crypto-rust` on Linux (`dbus-secret-service`, MIT OR Apache-2.0; secrets are encrypted on the bus; libdbus-1 is the same library tao already links). No target uses the crate's mock store in a shipped build — before this was declared per target, macOS and Linux builds silently kept every secret in memory and lost it at exit. Every keyring call goes through `accounts::keychain`, is serialised behind one process-wide gate and fails as its own error class (`Error::Keyring { op, details }`), so the UI says what the keyring could not do rather than "invalid key" or "IO error". The commands that read, write or delete a key run it off the main thread (`spawn_blocking`); the CurseForge key is read once per session into a cache warmed at startup off the main thread, so outbound requests never wait on the keyring. Absence and "could not read" are never folded: a key status the keyring cannot answer is `Unknown`, and a token or password it refuses to delete on Remove account / Delete server is reported to the user, not logged away.

## Part G — Desktop integration: launch arguments, inbound links, and shortcuts
...
1. **Launch arguments are trusted; `lucerna://` links are not.** Anything able to pass argv to `lucerna.exe` can already start arbitrary programs on that machine, so `--launch <instance> [--world … | --server …]` may start the game with no prompt — that is what a desktop shortcut uses. A URL scheme, by contrast, is reachable from any web page with a single navigation. Therefore **no `lucerna://` URL can launch anything**. This is also why desktop shortcuts are real `.lnk`/`.desktop` files carrying argv rather than `.url` files carrying a scheme URL, even though the latter would have been less code. Lucerna no longer registers the scheme itself (item 4), but a registration made by an earlier version can still be present on a machine, so both gates below remain load-bearing.
...
4. **Lucerna does not register the scheme; it only ever removes it.** Versions 0.21.0–0.24.x offered an opt-in toggle (Settings → Integrations) that wrote `HKCU\Software\Classes\lucerna` for the current user. Nothing ever produced `lucerna://` links, so the toggle was retired and every code path that created or updated the key was deleted with it. What remains is a one-time retirement at startup (`src-tauri/src/url_scheme_retire.rs`): the key is removed only when it is provably ours — its command names this very executable, or the user is on record as having opted in through this data root — and a key that points elsewhere with no such record is left alone. A failed removal keeps the consent record so the next start retries; settings that cannot be read count as "not opted in"; and a registry that cannot be read is not mistaken for "no key" — nothing is touched, consent record included, because clearing it would orphan a key that is really there. For a user who never enabled the toggle this costs one registry read and no write. The uninstaller applies the same rule to a key pointing into its own install directory. Removal stays confined to `platform::protocol`; the structural guard `src-tauri/tests/structural_platform_chokepoint.rs` fails the build if a registry-mutating call appears outside `src/platform/`.

5. **Scope.** `curseforge://` and `modrinth://` are deliberately **not** claimed — those schemes belong to those vendors' own applications.
