...
6. **Fifth sanctioned outbound channel — webview image loads (catalog icons, galleries, description images).** The production CSP in `src-tauri/tauri.conf.json` declares `img-src 'self' data: https:`, so the webview can load an `<img>` from any https host. That is an outbound channel the commitment-2 allowlist does not cover, and this document would be dishonest not to list it. What flows through it: project icons on catalog cards (`CardMedia`, `FindAlternativeDialog`, `ServerContentDetail`), gallery screenshots (`ImageGallery`), and images embedded in mod / modpack description bodies — Modrinth markdown and CurseForge HTML, sanitized by `ammonia` in `mods::render` before reaching the webview. Icons and galleries normally point at the platforms' own CDNs, but a description author chooses their own image hosts, so a hostile author can embed an image served from a host they control and learn the viewer's IP address, user agent, and the fact that the description was opened — a read-only beacon, the same exposure any web page's `<img>` carries. We accept that today rather than proxy every image through `network::` — the honest alternative, deliberately not taken: it would put an image-fetching proxy and cache into the backend to close a channel bounded to passive observation. The channel is bounded as follows:

   - **Webview-only, image-only.** The `https:` grant is on `img-src` alone. `connect-src` stays `'self' ipc: http://ipc.localhost`, so programmatic requests from webview code — fetch, XHR, WebSocket, beacons — cannot leave the machine in a production build, and `tools/check-no-network-calls.mjs` (run by `pnpm lint` and CI) fails the build if such a call appears in shipped webview source at all.
   - **No credentials.** Sign-in opens the system browser, never the app webview, so the webview holds no cookies or authentication for any remote host. An image request carries the IP address and user agent — nothing more.
   - **Sanitized HTML.** Description bodies pass through `ammonia` (`mods::render::sanitize_html`), which strips scripts, iframes, event-handler attributes, inline styles, and `javascript:` / `data:` URLs. An embedded image can observe that it was loaded; it cannot run code.
   - **https-only where we choose the URL.** Gallery URLs from the catalog APIs are filtered through `mods::render::is_safe_image_url`, which accepts `https://` only — defence in depth alongside the CSP.

7. **Microsoft and offline accounts are equal first-class citizens.** No UI warnings beyond honest technical disclosures (e.g., "offline accounts cannot connect to online-mode servers"). No "switch to a real license" suggestions. No moralizing copy. The launcher does not judge.

8. **Wire-level release self-audit (planned).** A CI integration test will boot the launcher in a controlled environment with a packet-capture tool, perform only "launch vanilla 1.20.x," and assert that every captured request targets an allowlisted host — an independent, out-of-process confirmation of the in-code enforcement in commitment 1. Status: not yet implemented; tracked in the project roadmap. See `docs/SECURITY.md` Part C.
...
| `explorer.exe` (or OS-default file manager) | Opens a user-clicked folder in the OS file manager — one Tauri command per folder: `open_instance_folder` (`<instance>/.minecraft/`), `open_mods_folder` (`mods/`), `open_saves_folder` (`saves/`), `open_screenshots_folder` (`screenshots/`), `open_backups_folder` (`<instance>/backups/<world>/`), `open_log_folder` (the folder of the log file being viewed, validated against the instance's log roots), `open_launcher_log_folder` (the launcher's own `logs/`), `open_imported_source_folder` (the folder an instance was imported from), `open_data_folder` (the effective data folder — refused in a recovery session and during a move, and checked first to still be the data folder, since on macOS and Linux the opener reports success even when nothing opens; the UI passes no path), `open_data_move_leftovers` (the previous data folder, right after a data-location move that could not remove everything from it; the backend remembers the path, the UI passes none); on macOS these two reveal a folder named `*.app` (the default data folder is `com.lucerna.app`) in its parent instead of opening it, because `open` would try to launch it as an application. And, for a server, `server_open_folder`, `server_open_logs_folder`, `server_open_plugins_folder`, `server_open_mods_folder` (its `runtime/` and the `logs/`, `plugins/`, `mods/` inside it) | `tauri_plugin_opener::OpenerExt::open_path` (`reveal_item_in_dir` for the macOS `*.app` case above); every call site is on the allowlist in `structural_no_raw_spawn.rs` | One-shot; the file manager opens (or focuses) a window and the spawn handle exits immediately | stdin/stdout/stderr not captured (GUI process) |
| `explorer.exe` (or OS-default file manager) | Reveals a specific screenshot in its folder (screenshot viewer) | `tauri_plugin_opener::OpenerExt::reveal_item_in_dir` via the `reveal_screenshot` Tauri command | One-shot; the file manager opens (or focuses) a window and the spawn handle exits immediately | stdin/stdout/stderr not captured (GUI process) |
...
| OS-default web browser | Opens an external web page the user acted on. Backend: the Microsoft sign-in page during account sign-in. Frontend: project pages on Modrinth, CurseForge, Hangar and FTB; manual-download pages for files whose authors disallow re-hosting; the OptiFine downloads page; the Minecraft EULA and the minecraft.net purchase page; the CurseForge API-key console; the AI-provider key pages (Anthropic, Google AI Studio, Groq) from Settings → AI translation; our GitHub repository and release pages; and `https` links inside changelog and mod-description text (descriptions are sanitized in Rust before they reach the webview; every frontend open goes through the https-only chokepoint) | Backend: `tauri_plugin_opener::OpenerExt::open_url` in `accounts::microsoft` (on the `structural_no_raw_spawn.rs` allowlist). Frontend: `openExternalHttps` in `src/lib/ui/safe-open.ts` — the one frontend chokepoint: it refuses every non-`https` scheme and reports a refused scheme or a failed open as a toast. `tools/check-opener-calls.mjs`, run as part of `pnpm lint`, fails if `@tauri-apps/plugin-opener` is referenced anywhere else (the shortcut dialog's `revealItemInDir` is the one other allowed reference) | One-shot; the browser opens (or focuses) a tab and the spawn handle exits immediately | stdin/stdout/stderr not captured (GUI process) |
...
| `structural_no_heavy_sync_command.rs` | A `#[tauri::command]` whose body carries any known heavy-work call — the mods/plugins reconcile scan, archive extract/zip, recursive tree delete or copy, a recursive size walk, image decode/encode, or a `process::` subprocess — must be `async` with the blocking work in `spawn_blocking`. A synchronous command runs on the main thread and freezes the window for the duration. Line-scoped, stale-checked allowlist; a command that delegates its heavy work to a helper is invisible to a lexical scan and stays review's job |
| `structural_no_inplace_mods_write.rs` | No raw write primitive — `fs::write`, `fs::copy`, or opening for write via `File::create` / `File::options` / `OpenOptions` — under `src/mods/`, `src/datapacks/`, or `src/worlds/` outside `mods::store` (instance side) and `mods::cache` (store side); importing one of those names unqualified is itself a violation, so the call-site matchers keep seeing them. Instance mod jars and world datapack links are hardlinks to one shared physical file, so an in-place write — `fs::copy` included, since it opens the destination with truncate — corrupts every instance or world sharing it. Only write-to-temp-then-rename is safe. Test-module interiors are exempt (fixture writes are not production writes); production code before, between, and after them stays scanned. |
| `structural_no_blind_err_swallow.rs` | Four rules against a blind failure swallow: no `Err` arm with an empty body (a match guard naming the error is the discrimination this asks for, and passes); no discarded `Result` from an fs `rename`/`write` without a justification comment; no one-line `let Ok(..) = <fs read> .. else` whose body answers `return Ok(..)` — every failure, `NotFound` and "could not tell" alike, laundered into success (no comment escape; the guard's line-scoped, stale-checked allowlist is the only hatch); and no `.unwrap_or_default()` chained onto an fs read on the same line without a justification comment. Importing `fs::rename`/`fs::write` unqualified is itself a violation, so the matchers keep seeing call sites. Removals are excluded — see the module doc. Enforces the mechanically-checkable part of principle B.7's last two bullets; a swallowed *removal* on a recovery path and a promised-but-missing log remain review's job. |
| `structural_maintenance_gate.rs` | Every instance, world and datapack writer that a world migration's maintenance claim must hold off — the mutating world commands and `delete_backup`, the datapack commands' file-local delegate, `delete_instance` / `rename_instance_dir` / `change_instance_mc` / `clone_instance`, and launch both at command entry and again inside `launch::start` after `claim_start` — calls `instances::maintenance::write_allowed` (launch and `delete_backup`: `maintenance_is_active`) on a code line of its body; and every `#[tauri::command]` in `commands/worlds.rs` and `commands/datapacks.rs` is either gated, takes the claim itself, or is declared read-only with a reason, so a new command must choose in code. Lexical: it proves the gate is called, not that it runs before the first write; ordering, and additions to `commands/instances.rs`, stay review's job |
| `structural_server_sidecar_lock.rs` | The server content sidecars (`servers_runtime::installed` for a server's mods and plugins, `servers_runtime::datapacks::sidecar` for a world's datapacks) are rewritten under their read-modify-write lock. `load`/`save` exist only as methods of the lock guard, so an unlocked read or write does not compile; the guard adds what the types cannot see: the lock is taken once per function as a named local — never a temporary, never `let _` — no holder calls another function that takes it (it is not re-entrant), and no other file names the lock. Concurrent installs into one server otherwise lose each other's rows |
| `structural_platform_chokepoint.rs` | A fixed list of OS-divergent primitives — `PermissionsExt`, `WaitForInputIdle`, `libc::kill`, and the registry-mutating `Reg*W` calls — appears nowhere outside `src/platform/`. It does not police `#[cfg(windows)]` in general; keeping broader OS-specific behaviour behind the seam remains a review responsibility |
| `structural_no_env_mutation.rs` | No `std::env::set_var` / `remove_var` anywhere — production and test code alike, `src/` and `tests/` both scanned — env overrides go through `test_seam` (this is what removed the need for single-threaded test runs) |
| `structural_installer_branding.rs` | The NSIS installer keeps Lucerna branding and localization wired up, and the never-published MSI target stays out of `bundle.targets` |
