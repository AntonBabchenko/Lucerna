...
`src/lib/ipc/bindings.ts` is generated by tauri-specta from the Rust command/type signatures — never hand-edit it. The `export_bindings` binary drives the same `specta_builder()` the app uses, so it regenerates without launching the app:
...
This is the no-dev-server path — it never starts the app, opens a window, or binds the Vite port (the only other way the file gets written is the `#[cfg(debug_assertions)]` export at `pnpm tauri dev` startup). It must run via `cargo run` (not as a `tests/` binary): the exporter links the webview runtime and has to run from `target/<profile>/` where the loader library sits, not from `target/<profile>/deps/`.
