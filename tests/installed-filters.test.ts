...
const row = (sha1: string, name: string, enabled: boolean, source: ModSource | null = 'modrinth'): Row => ({
  summary: null,
  installed: {
    filename: `${sha1}.jar`, sha1, source, project_id: source ? 'p' + sha1 : null,
    version_id: 'v', name, version_number: '1.0', installed_at: `2026-01-0${sha1}T00:00:00Z`,
    enabled, enrich_attempted: false,
  },
});
...
describe('createInstalledFilters', () => {
  it('counts total/enabled/disabled over the unfiltered rows', () => {
    ...
    const f = createInstalledFilters(() => rows, () => new Set(), () => new Set());
  ...
  it('quickFilter "updates" keeps only updatable shas', () => {
    ...
    const f = createInstalledFilters(() => rows, () => new Set(['2']), () => new Set());
  ...
  it('quickFilter "issues" keeps only shas with missing deps', () => {
    const rows = [row('1', 'A', true), row('2', 'B', true)];
    const f = createInstalledFilters(() => rows, () => new Set(), () => new Set(['1']));
