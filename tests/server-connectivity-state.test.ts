...
vi.mock('$lib/ipc/bindings', () => ({
  ...
  events: {
    serverLogLine: { listen }, serverSpawned: { listen },
    serverExited: { listen }, serverUploadProgress: { listen },
  },
}));
...
describe('serverState.connectivity', () => {
  ...
  it('returns the connectivity snapshot on ok', async () => {
    connectivity.mockResolvedValue({ status: 'ok', data: { lan_addresses: ['192.168.1.5'], port: 25565, online_mode: true } });
  ...
  it('returns null on error', async () => {
    connectivity.mockResolvedValue({ status: 'error', error: { kind: 'io', path: 'p', details: 'd' } });
