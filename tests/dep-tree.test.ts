...
const tree: DepTreeNode[] = [
  { source: 'modrinth', project_id: 'arch', name: 'Architectury', status: 'missing_required', cycle: false, children: [] },
  { source: 'modrinth', project_id: 'night', name: 'Night', status: 'satisfied', cycle: false, children: [] },
];

describe('DepTree', () => {
  it('renders missing nodes with an Install action', async () => {
    const onInstall = vi.fn();
    render(DepTree, { props: { nodes: tree, hoveredKey: null, onHover: () => {}, onInstall, onAdd: () => {} } });
