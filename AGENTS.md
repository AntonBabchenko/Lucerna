...
- Any write into an instance's content directories outside the `mods::store` chokepoint — instance mod jars are hardlinks to one shared physical file, so an in-place write (including `fs::copy`, which truncates its destination) corrupts every instance using that mod (caught by `src-tauri/tests/structural_no_inplace_mods_write.rs`).
